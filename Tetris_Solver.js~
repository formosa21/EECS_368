...
/*  most left and lowest of that column
    ...
    hmlb: how many columns to the left towards the boundary (exclusive to the left element)
*/
    ...
    for(var i = 0; i < 4; i++){
        ...
        for(var j = 0; j < cols; j++){
            ...
            if(land_ok_for_left_lower_most_AND_no_crossover(grid, position, j, blocktype, i)){
                var result = calculate_left_lower_level(grid, j, blocktype, i);
    			...
    			if(blocktype["mark"] == 2|| blocktype["mark"] == 3 || blocktype["mark"] == 4){
    				...
    				if(j < column_select){
    					if(result < most_optimal || (result-1) < most_optimal){
    							most_optimal = result;
    ...
    */
    ...
    for(var j = 0; j < 4; j++){
        ...
        for(var a = 3-blocktype["hmlb"][ith_shape]; a < 4; a++){
        	if((k+a)%10 == 0){ // cross the right boundary ~ break
        		if(k%10 != 0)
        			break;
        	}
        ...
        for(var a =four_by_four_start; a < 4; a++){
        	if(four_by_four_start > 0)
            	if(position_code[a+line] == 1)
                	grid[top_left_corner +j] = "f";
            if(four_by_four_start == 0)
 				if(position_code[a+line] == 1)
                	grid[top_left_corner +j + a] = "f";
        }
        line += 4;
    }
...
tetrisGame.AddShape(4,4,1);
while(tetrisGame.IsShapeFalling()){
	tetrisGame.IncrementTime();
}/*
tetrisGame.AddShape(2,4,2);
while(tetrisGame.IsShapeFalling()){
	tetrisGame.IncrementTime();
}
tetrisGame.AddShape(5,4,3);
while(tetrisGame.IsShapeFalling()){
	tetrisGame.IncrementTime();
}*/
...
print_grid(tetrisGame.currentState);
